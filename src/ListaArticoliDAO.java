...
public class ListaArticoliDAO{
	...
	public ArrayList<Articolo> ListaArticoli(){
		...
		try {
			...
			while(result.next())
			{
				A = new Articolo(result.getString("idarticolo"), result.getString("nome"), result.getString("taglia"), result.getString("colore"), result.getInt("scorte"), result.getDouble("prezzo"));
				ap.add(A);
			}
